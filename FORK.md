...
- `crates/codegen/xai-grok-update/src/auto_update.rs` — #405: the fork owns `reinstall_hint()`, which must point at the medley installer; telling a fork user to `npm i -g @xai-official/grok` is exactly the silent-replacement failure that module exists to prevent. Upstream calls it itself (`eb267fef`, at `:979`) and changed its signature to `(installer, channel)` this round, against the fork's one-argument form — so the collision is on the definition *and* the call. **Irreducible** for the same reason as above, and it takes the same rerere resolution: keep *ours* for the definition, re-apply upstream's non-state-dir changes on top.

#405 measured three rebrand-caused conflicts against upstream `eb267fef`. The two above are the irreducible ones. The third — the alphabetised `pub use paths::{ .. }` block in `crates/codegen/xai-grok-config/src/lib.rs` — was removable and is gone: `pin_grok_home` now sits on its own re-export line so the block stays byte-identical to the merge base, and upstream's additions to it merge cleanly. A comment in that file says so; do not fold it back in.
