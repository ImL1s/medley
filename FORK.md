...
- `crates/codegen/xai-grok-update/src/auto_update.rs` — #405: conflicts every sync, but the two halves of it resolve **in opposite directions**, so read the hunk before reaching for *ours*. Against `eb267fef` the file has five conflict hunks. Four are the fork's `dist_channel::self_update_refusal()` guards — inserted into upstream functions (`run_update_if_available`, `run_install_script`, `install_internal_from_bases`, `refresh_deployment_config`) so a medley build never runs the inherited updater. Those are **irreducible**: they live inside code upstream owns and keeps editing. Keep *ours* there and re-apply upstream's changes on top — the same resolution every sync, which is what makes `rerere` (above) worth enabling.
