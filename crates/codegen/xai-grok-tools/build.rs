...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    bundle_rg()?;
    // fd backs the PI glob contract only; default builds keep the existing rg path.
...
/// ripgrep bundling (release-only or `GROK_TOOLS_BUNDLE_FD_PATH` override),
/// plus pinned SHA-256 verification of the downloaded tarball.
fn bundle_fd() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The runtime extraction is unix-only; never embed on Windows targets.
