...
Sample hooks for Grok. Copy to `~/.medley/hooks/` to enable globally, or to `<project>/.grok/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.medley/hooks/bin
cp examples/hooks/safe-shell.json ~/.medley/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.medley/hooks/bin/
chmod +x ~/.medley/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.medley/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.medley/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.medley/hooks/bin/
chmod +x ~/.medley/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.medley/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.medley/hooks/bin
cp examples/hooks/session-log.json ~/.medley/hooks/
cp examples/hooks/bin/session-log.sh ~/.medley/hooks/bin/
chmod +x ~/.medley/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.medley/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.medley/hooks/bin
cp examples/hooks/tool-logger.json ~/.medley/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.medley/hooks/bin/
chmod +x ~/.medley/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.medley/hooks/bin
cp examples/hooks/stop-verify.json ~/.medley/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.medley/hooks/bin/
chmod +x ~/.medley/hooks/bin/stop-verify.sh
```
...
Remove the JSON file from `~/.medley/hooks/`. The hook stops running on the next session.