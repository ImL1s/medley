...
/// Absolute path of the bundled user guide, for the system prompt.
///
/// Resolved rather than written literally because the state directory moves:
/// `MEDLEY_HOME` can point anywhere, and an install that kept an existing
/// `~/.grok` resolves there instead. The prompt hands this to the model as a
...
mod tests {
    ...
    /// The user guide path in the system prompt has to *move* with the state
    /// directory. The bug this replaces was a literal `~/.grok/docs/user-guide/`
    /// baked into the prompt: on a `~/.medley` install the model was told to
    /// read a directory that does not exist, found nothing, and answered from
    /// memory instead — wrong answers, no error, nothing to notice.
    ///
    /// Asserting two different homes give two different answers is the point:
    /// a constant satisfies "looks like a path" and fails this.
    #[test]
    fn the_user_guide_path_follows_the_state_directory() {
        assert_eq!(
            user_guide_dir_in(Path::new("/home/a/.medley")),
            "/home/a/.medley/docs/user-guide"
        );
        assert_eq!(
            user_guide_dir_in(Path::new("/home/a/.grok")),
            "/home/a/.grok/docs/user-guide",
            "an install that kept an existing ~/.grok must be pointed there, \
             not at ~/.medley"
        );
        assert_ne!(
            user_guide_dir_in(Path::new("/srv/one")),
            user_guide_dir_in(Path::new("/srv/two")),
            "a constant would pass every assertion above and fail this one"
        );
    }

    /// And the template has to actually consume it. `prompt.md` is compiled in
    /// as an encrypted blob regenerated by `scripts/encrypt_templates.py`, so
    /// editing the markdown without regenerating leaves the old literal
    /// shipping — a silent no-op this catches.
    #[test]
    fn the_rendered_prompt_names_the_resolved_guide_directory() {
        let rendered = render_base_template(minijinja::context! {
            ..base_template_ctx(),
            ..minijinja::context! { user_guide_dir => "/srv/state/docs/user-guide" }
        });

        assert!(
            rendered.contains("/srv/state/docs/user-guide"),
            "the resolved path never reached the prompt"
        );
        assert!(
            !rendered.contains("~/.grok/docs/user-guide"),
            "the hardcoded literal is still in the compiled template — \
             rerun scripts/encrypt_templates.py"
        );
    }

    fn render_base_template(ctx: minijinja::Value) -> String {
