...
Place this file at `~/.medley/settings.json` or `~/.claude/settings.json`.
...
Set these in `~/.medley/config.toml`:
...
To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.medley/pager.toml`.
...
For a hands-off install with no per-person step, also place the plugin's files where Grok discovers and trusts them automatically: `~/.medley/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.medley/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).
...
| `~/.medley/plugins/` | User, every project | Trusted automatically |
