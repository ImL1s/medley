...
Clients connect over WebSocket and authenticate with a secret token supplied by
`--secret` or `GROK_AGENT_SECRET` (with one interactive-loopback exception described
below).

Clients should send the token in the `Authorization: Bearer <token>` header. The legacy
`?server-key=<token>` query parameter is accepted only for loopback listeners and is
deprecated because URLs are commonly retained in browser history, proxy logs, and
diagnostics. Never put a server secret in a URL.

On a loopback address, an interactive terminal may omit `--secret`; Grok generates a
256-bit token and writes it once to that terminal. Non-interactive runs must supply
`--secret` or `GROK_AGENT_SECRET` so credentials cannot appear unexpectedly in logs.

Binding beyond loopback is an explicit security boundary. It requires both
`--allow-remote` and an explicit, cryptographically random secret of at least 32
bytes (for example, a 256-bit token generated by Grok during a loopback run):
...
The listener uses plaintext `ws://`. Do not expose it directly to an untrusted network.
Terminate TLS in an operator-managed reverse proxy and connect with `wss://`, or keep
the agent on loopback and use an SSH tunnel:
