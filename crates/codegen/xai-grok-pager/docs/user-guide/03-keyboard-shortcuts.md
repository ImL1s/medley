...
Reference for key bindings in the Medley TUI. Bindings are built in and cannot currently be remapped.
...
Simple mode is active by default. To switch to Vim mode, set `vim_mode = true` under `[ui]` in `~/.medley/config.toml`, or toggle it at runtime with `/vim-mode`. See [Configuration](05-configuration.md) for details.
...
> in `~/.medley/config.toml` (or `/vim-mode` to toggle). Arrow keys, `Tab`,
