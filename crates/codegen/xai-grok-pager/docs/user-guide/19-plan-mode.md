...
The plan is written to `plan.md` inside the session directory (`~/.medley/sessions/<cwd>/<session-id>/plan.md`, where `<cwd>` is an encoded directory name, not the literal path).
