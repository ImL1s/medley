...
### Agent server network boundary

`agent serve` defaults to loopback. Authenticate clients with the
`Authorization: Bearer <token>` header; never place the token in a URL. The legacy
`server-key` query parameter is loopback-only and deprecated.

For non-loopback binds, Grok requires `--allow-remote` plus an explicit,
cryptographically random `--secret` or `GROK_AGENT_SECRET` of at least 32 bytes
(for example, a 256-bit token generated by Grok during a loopback run). This flag
acknowledges network exposure; it does not add encryption. The listener is plaintext `ws://`, so use an operator-managed TLS
reverse proxy for `wss://` or tunnel a loopback listener over SSH:

```bash
# Remote bind behind an operator-managed TLS reverse proxy
GROK_AGENT_SECRET='<token>' grok agent --always-approve serve \
  --bind 0.0.0.0:2419 --allow-remote

# Safer direct administration: keep Grok on loopback and tunnel it
ssh -N -L 2419:127.0.0.1:2419 user@agent-host
```

Interactive loopback startup may generate a 256-bit secret and display it once in that
terminal. Non-interactive startup never generates or prints a secret and therefore
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.medley/`, or system-wide under `/etc/grok/` for enforcement users cannot remove):
...
|-------|------|-----------------------|
| Global (all projects) | `~/.medley/config.toml` | No |
...
### 2. Native Configuration (`~/.medley/config.toml` and `.grok/config.toml`)
...
```bash
chmod +x ~/.medley/hooks/git-gh-only.sh
```
