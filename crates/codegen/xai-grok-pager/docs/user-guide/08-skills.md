...
| `~/.medley/skills/`, `~/.medley/commands/` | User | Lowest | Personal skills for all projects |
...
Grok scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.medley/config.toml`, or set the `GROK_CURSOR_SKILLS_ENABLED` or `GROK_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Grok always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.

### Additional Skill Directories

Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.medley/config.toml`:
...
- **User** (`~/.medley/skills/<name>/`) -- available across all your projects.
...
/user:commit         # The "commit" skill from ~/.medley/skills/
...
Grok distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.medley/bundled/skills/`; Grok never writes them into `~/.medley/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `grok inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)
...
5. **Version-control project skills.** Commit `.grok/skills/` to your repository so the whole team benefits. User skills in `~/.medley/skills/` stay personal and unshared.
