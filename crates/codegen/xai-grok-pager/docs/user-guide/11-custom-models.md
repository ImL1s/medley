...
> [!NOTE]
> **Keyless routes (`auth_scheme = "none"`) do not get the local `web_search` tool.** This is decided behavior, not a gap (see issue [#178](https://github.com/ImL1s/medley/issues/178)). The local tool sends its own outbound request carrying a hosted web-search tool, and no keyless local runtime — Ollama, llama.cpp, vLLM, LM Studio — implements one; enabling it would swap a clear disable notice for a request-time failure you could not act on. The notice names the scheme rather than reporting a missing credential. For a keyless endpoint that genuinely does serve server-side search, the supported path is `supports_backend_search = true` with `api_backend = "responses"`, described above — that route needs no credential check, because the search runs inside the sampling request the model route already makes.
