...
MCP servers are configured in `~/.medley/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.medley/config.toml`) **or repo-level**
...
`grok mcp enable` / `disable` persist the personal on/off state to user `~/.medley/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:
...
|----------|-------|----------|
| `~/.medley/config.toml` | All projects | Lowest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.medley/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.medley/config.toml`, `.grok/config.toml` | Always on |
...
For stdio servers, Grok captures the process's standard error to `~/.medley/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.medley/logs/mcp/filesystem.stderr.log
```
