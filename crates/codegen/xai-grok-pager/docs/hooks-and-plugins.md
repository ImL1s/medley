...
Medley supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.medley/hooks/`
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.medley/settings.json` or `~/.claude/settings.json`
...
Sources land in `~/.medley/config.toml`:
...
Or in `~/.medley/settings.json` / `~/.claude/settings.json`:
