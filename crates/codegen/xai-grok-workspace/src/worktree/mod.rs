...
/// Resolve the state directory for worktree paths via the **same** resolver used
/// for `worktrees.db` (`xai_fast_worktree::resolve_grok_home`), so checkout dirs
/// and the metadata DB always live under the same tree. Both that resolver and
/// the no-home fallback route through `xai_grok_config`, so worktree paths also
/// agree with trust/hooks and every other state path.
