...
impl JsonlStorageAdapter {
    ...
    ///
    /// The behaviour lives in `xai_grok_workspace`, shared with the fresh-claim
    /// gate in `session::persistence` (#412). This adapter only supplies the
    /// publication parent, and only in `FromRoot` mode: an `Explicit` binding
    /// names a directory outside the `sessions/<encoded-cwd>/<id>` namespace,
    /// where there is no session id to free.
