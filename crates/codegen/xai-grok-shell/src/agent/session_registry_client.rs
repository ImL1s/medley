...
impl SessionRegistryClient {
    ...
    pub async fn register(&self, req: &RegisterRequest) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session register"));
        }
        Ok(())
    }
    ...
    pub async fn update(&self, session_id: &str, req: &UpdateRequest) -> Result<()> {
        ...
        let (response, stamp) = self
            ...
            .await?;
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session update"));
        }
        Ok(())
    }
    ...
    pub async fn finalize(&self, session_id: &str) -> Result<()> {
        ...
        let (response, stamp) = self
            ...
            .await?;
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session finalize"));
        }
        Ok(())
    }
    ...
    pub async fn search(&self, query: Option<&str>, limit: i64) -> Result<Vec<SessionRecord>> {
        ...
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session search"));
        }
    ...
    pub async fn get_session(&self, session_id: &str) -> Result<SessionRecord> {
        ...
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session get"));
        }
    ...
    pub(crate) async fn get_download_url(
        ...
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session download url"));
        }
    ...
    pub async fn download_file(
        ...
        if !response.status().is_success() {
            return Err(self.check_response(response, Some(&stamp), "session download"));
        }
        let resp: DownloadResponse = response.json().await.context("parse download response")?;
