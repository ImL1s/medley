...
//! user into the provider's sign-in flow instead of advertising a dead bearer.
//!
...
//! than no credential and `cached_token` was advertised at startup.
//!
//! Mid-session backend rejection is covered at the in-crate actor seam by
//! `external_session_401_surfaces_provider_login_retry_state`. It cannot use
//! this cleartext loopback integration server: production correctly refuses to
//! attach an ambient session credential to that untrusted origin.
//!
...
async fn connect(client_type: &str) -> (acp::ClientSideConnection, acp::InitializeResponse) {
    // The fixture's loopback `/v1/models` endpoint is deliberately public.
    // Declare that explicitly instead of weakening the production custom-
    // catalog gate or adding an XAI_API_KEY that would mask session auth.
    let raw_config: toml::Value = toml::from_str(
        r#"[models]
catalog_auth_scheme = "none"
"#,
    )
    .expect("parse mock catalog auth config");
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .expect("valid unauthenticated mock catalog config");
