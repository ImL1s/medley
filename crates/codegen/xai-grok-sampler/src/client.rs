...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Codex traffic must truthfully identify this product. `origin_client`
        // is public per-session input and must not be able to impersonate the
        // official Codex CLI on this transport.
        {
            let ua_string = if is_codex {
                ...
                match config.origin_client.as_ref() {
                    ...
                    None => grok_build_user_agent_string(),
                }
            };
