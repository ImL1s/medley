...
pub enum SamplingEvent {
    /// Images were stripped due to model restrictions.
    ImagesStripped {
        request_id: RequestId,
        stripped_urls: Vec<std::sync::Arc<str>>,
        reason: crate::StripReason,
    },
...
mod tests {
    ...
    fn sampling_error_info_debug_omits_message_and_credential_details() {
        ...
        let info = SamplingErrorInfo {
            ...
            is_retryable: false,
            // Upstream added this field; irrelevant to what this test asserts
            // (that `Debug` omits the message and the credential), so neutral.
            should_retry: None,
            error_code: None,
            retry_after_secs: None,
            model_metadata: None,
