...
CI's hot path is exactly this suite: `*_is_secret_free_*` (3), `omits_xai_identity` (4), `hostile_injector` (3), `none_auth_scheme_` (5), and three tests named explicitly rather than by pattern — `sampler_request_logs_never_emit_credential_bytes`, `transport_failure_never_emits_query_credential_bytes`, `subagent_resolution_diagnostics_never_emit_parent_or_child_credentials`. The obvious single pattern for that family, `never_emit_credential_bytes`, selected only the first of the three (#487): no substring narrower than bare `never_emit` reaches all three, and that one also selects 6 unrelated tests elsewhere in the tree. If a change is going to break something, that is where it shows.

All five counts are measured against every test target a crate has (`--lib` **and** its integration `tests/` targets) — a `--lib`-only measurement is exactly how #487's own investigation first undercounted 3 of these 5 — and pinned by `tests.test_credential_hot_path_guard`: a shrinking count reddens CI before it reddens someone's trust in a `1 passed` report.
